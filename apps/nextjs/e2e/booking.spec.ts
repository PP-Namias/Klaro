...
test.describe("Patient Booking Flow", () => {
  test("patient can view facilities listing", async ({ page }) => {
    ...
    await page.screenshot({ path: "e2e-screenshots/11-facilities-listing.png" });
  });
  ...
  test("patient can view the API health endpoint", async ({ page }) => {
    const response = await page.request.get("/api/trpc/facilities.searchNearby?input=%7B%22json%22%3A%7B%22latitude%22%3A14.5995%2C%22longitude%22%3A120.9842%7D%7D");
