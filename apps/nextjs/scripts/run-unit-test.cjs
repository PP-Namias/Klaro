...
if (failed.length > 0) {
  console.error('Unit test failed - missing expected camera-first layout elements:', failed);
  process.exit(2);
} else {
  console.log('✓ Unit test passed: camera-first layout with image-matching design verified.');
  process.exit(0);
}
